    ...
    async def gather(self) -> Tuple[List[RoundMember], Dict[str, AggResultType]]:
        ...
        for retry_time in range(5):
            try:
                ...
                _logger.warning('select clients failed')
                await asyncio.sleep(20)
                if retry_time == 4:
                    raise ValueError("select clients failed")
        await self.select_candidates(u1)

        # wait for clients to upload secret shares
        await asyncio.sleep(self.strategy.connection_timeout * 2)
        ...
        for retry_time in range(5):
            try:
                ...
                _logger.warning('get clients failed')
                await asyncio.sleep(20)
                if retry_time == 4:
                    raise ValueError("get clients failed")
        # start calculation
